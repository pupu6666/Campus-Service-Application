...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    /*
    // 限流和熔断处理方法
    public Result blockHandler(Long voucherId, BlockException ex) {
        return Result.fail("请求被限流或熔断，请稍后再试");
    }

    // 熔断降级处理方法
    public Result fallback(Long voucherId) {
...
//        }
    ...
    @SentinelResource(value = "seckillVoucher", blockHandler = "blockHandler", fallback = "fallback")
    @Override
    ...
    public Result seckillVoucher(Long voucherId) {
        ...
        // 将 VoucherOrder 对象转换为 JSON 字符串
        String orderJson = null;
        try {
            orderJson = objectMapper.writeValueAsString(voucherOrder);
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
        }
        // 发送 JSON 字符串到 Kafka
        kafkaTemplate.send("voucher-orders", orderJson);
